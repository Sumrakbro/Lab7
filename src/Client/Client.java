    ...
    private static int SendData(DatagramChannel channel, InetAddress serverIP) {
        try {
            if (serverIP.isReachable(1000)) {
                channel.connect(new InetSocketAddress(serverIP, 7778));
                ...
                if (command instanceof Command_Exit) {
                    channel.send(ByteBuffer.wrap(Serializer.serialize(new Command_Disconnect())), new InetSocketAddress(serverIP, 7778));
                    System.exit(0);
               ...
               if(command instanceof Command_Add) ((Command_Add) command).setLogin(login);
                ...
                channel.send(ByteBuffer.wrap(Serializer.serialize(command)), new InetSocketAddress(serverIP, 7778));
